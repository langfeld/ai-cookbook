<!--
  ...
  ============================================
  Modal zum Exportieren und Importieren von Rezept-Sperren.
      ...
      <div class="z-10 relative bg-white dark:bg-stone-900 shadow-2xl rounded-2xl w-full max-w-lg max-h-[90vh] overflow-hidden">
        ...
        <div class="flex justify-between items-center p-5 border-stone-200 dark:border-stone-700 border-b">
          <h2 class="font-display font-bold text-stone-800 dark:text-stone-100 text-xl">
            {{ isAdmin ? '🚫 Admin Rezept-Sperren' : '🚫 Rezept-Sperren Export/Import' }}
          </h2>
        ...
        <div class="p-5 max-h-[60vh] overflow-y-auto">
          ...
          <div v-if="activeTab === 'export'" class="space-y-4">
            <p class="text-stone-600 dark:text-stone-400 text-sm">
              {{ isAdmin ? 'Exportiere alle Rezept-Sperren als JSON-Datei.' : 'Exportiere deine Rezept-Sperren als JSON-Datei.' }}
            </p>
            ...
            >
              ...
              {{ exporting ? 'Exportiere...' : 'Sperren exportieren' }}
            </button>
          </div>
          ...
          <div v-if="activeTab === 'import'" class="space-y-4">
            <p class="text-stone-600 dark:text-stone-400 text-sm">
              Importiere Rezept-Sperren aus einer JSON-Datei. Bestehende Sperren werden aktualisiert.
            </p>
            ...
            <div v-if="filePreview" class="bg-stone-50 dark:bg-stone-800 p-4 rounded-lg">
              ...
              <div class="space-y-1 text-stone-600 dark:text-stone-400 text-sm">
                <p>{{ filePreview.block_count }} Rezept-Sperren</p>
                <p v-if="filePreview.exported_at">Exportiert: {{ formatDate(filePreview.exported_at) }}</p>
                <div v-if="filePreview.sample_titles?.length" class="mt-2">
                  <p class="text-stone-500 text-xs">Gesperrte Rezepte:</p>
                  <p v-for="t in filePreview.sample_titles" :key="t" class="text-xs">• {{ t }}</p>
                </div>
              </div>
            </div>
            ...
            >
              ...
              {{ importing ? 'Importiere...' : 'Sperren importieren' }}
            </button>
...
async function handleExport() {
  ...
  try {
    const base = props.isAdmin ? '/api/admin/export/recipe-blocks' : '/api/recipe-blocks/export';
    ...
    a.href = url;
    a.download = `rezept-sperren-export-${new Date().toISOString().split('T')[0]}.json`;
    a.click();
    URL.revokeObjectURL(url);
    showSuccess('Rezept-Sperren erfolgreich exportiert!');
  } catch (err) {
  ...
  try {
    ...
    if (data.blocks?.length) {
      filePreview.value = {
        ...
        sample_titles: data.blocks.slice(0, 5).map(b => b.recipe_title || `Rezept #${b.recipe_id}`),
      };
    } else {
      filePreview.value = null;
      showError('Keine Rezept-Sperren in der Datei gefunden.');
    }
  } catch {
    filePreview.value = null;
...
async function handleImport() {
  ...
  importResult.value = null;
  try {
    const base = props.isAdmin ? '/admin/import/recipe-blocks' : '/recipe-blocks/import';
