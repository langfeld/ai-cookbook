<!--
  ...
  ============================================
  Modal zum Exportieren und Importieren von Einkaufslisten.
      ...
      <div class="z-10 relative bg-white dark:bg-stone-900 shadow-2xl rounded-2xl w-full max-w-lg max-h-[90vh] overflow-hidden">
        ...
        <div class="flex justify-between items-center p-5 border-stone-200 dark:border-stone-700 border-b">
          <h2 class="font-display font-bold text-stone-800 dark:text-stone-100 text-xl">
            {{ isAdmin ? '🛒 Admin Einkaufslisten' : '🛒 Einkaufslisten Export/Import' }}
          </h2>
        ...
        <div class="p-5 max-h-[60vh] overflow-y-auto">
          ...
          <div v-if="activeTab === 'export'" class="space-y-4">
            <p class="text-stone-600 dark:text-stone-400 text-sm">
              {{ isAdmin ? 'Exportiere alle Einkaufslisten als JSON-Datei.' : 'Exportiere deine Einkaufslisten als JSON-Datei.' }}
            </p>
            ...
            >
              ...
              {{ exporting ? 'Exportiere...' : 'Einkaufslisten exportieren' }}
            </button>
          </div>
          ...
          <div v-if="activeTab === 'import'" class="space-y-4">
            <p class="text-stone-600 dark:text-stone-400 text-sm">
              Importiere Einkaufslisten aus einer JSON-Datei. Importierte Listen werden als inaktiv angelegt.
            </p>
            ...
            <div v-if="filePreview" class="bg-stone-50 dark:bg-stone-800 p-4 rounded-lg">
              ...
              <div class="space-y-1 text-stone-600 dark:text-stone-400 text-sm">
                <p>{{ filePreview.list_count }} Einkaufslisten</p>
                <p>{{ filePreview.total_items }} Artikel insgesamt</p>
                ...
                <div v-if="filePreview.sample_names?.length" class="mt-2">
                  <p class="text-stone-500 text-xs">Beispiel-Listen:</p>
                  <p v-for="n in filePreview.sample_names" :key="n" class="text-xs">• {{ n }}</p>
                </div>
              </div>
            </div>
            ...
            >
              ...
              {{ importing ? 'Importiere...' : 'Einkaufslisten importieren' }}
            </button>
...
async function handleExport() {
  ...
  try {
    const base = props.isAdmin ? '/api/admin/export/shopping-lists' : '/api/shopping/export';
    ...
    a.href = url;
    a.download = `einkaufslisten-export-${new Date().toISOString().split('T')[0]}.json`;
    a.click();
    URL.revokeObjectURL(url);
    showSuccess('Einkaufslisten erfolgreich exportiert!');
  } catch (err) {
...
async function processFile(file) {
  ...
  try {
    ...
    if (data.lists?.length) {
      filePreview.value = {
        list_count: data.lists.length,
        total_items: data.lists.reduce((sum, l) => sum + (l.items?.length || 0), 0),
        exported_at: data.exported_at,
        sample_names: data.lists.slice(0, 5).map(l => l.name || 'Ohne Name'),
      };
    } else {
      filePreview.value = null;
      showError('Keine Einkaufslisten in der Datei gefunden.');
    }
  } catch {
    filePreview.value = null;
...
async function handleImport() {
  ...
  importResult.value = null;
  try {
    const base = props.isAdmin ? '/admin/import/shopping-lists' : '/shopping/import';
